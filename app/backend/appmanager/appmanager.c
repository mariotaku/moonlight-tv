...
void application_manager_load(PSERVER_LIST node) {
    ...
    node->appload = true;
    pthread_create(&_application_manager_load_thread, NULL, (void *(*)(void *)) _application_manager_applist_action,
                   node);
}
...
void appmanager_unregister_callbacks(PAPPMANAGER_CALLBACKS callbacks) {
    assert(callbacks);
    PAPPMANAGER_CALLBACKS find = appmanager_callbacks_find_by(callbacks_list, callbacks,
                                                              appmanager_callbacks_comparator);
    if (!find)
        return;
