...
static realloc_func _realloc_orig = &realloc;
...
static void *relloc_logged(void *p, size_t newsize, const char *file, const char *func, int line) {
    void *ret = _realloc_orig(p, newsize);
    fprintf(logfile(), "%s:%d(%s) free %p\n", file, line, func, p);
    fprintf(logfile(), "%s:%d(%s) malloc %p %d\n", file, line, func, ret, newsize);
    return ret;
}
...
#define realloc(p, newsize) realloc_logged(p, newsize, __FILENAME__, __FUNCTION__, __LINE__)
