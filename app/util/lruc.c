...
uint32_t lruc_hash(lruc *cache, int key) {
...
int lruc_cmp_keys(lruc_item *item, int key) {
    return key - item->key;
}
...
#define test_for_missing_key()        error_for(!key, LRUC_MISSING_KEY)
...
lruc_error lruc_set(lruc *cache, int key, void *value, uint32_t value_length) {
  ...
  while(item && lruc_cmp_keys(item, key)) {
    prev = item;
...
lruc_error lruc_delete(lruc *cache, int key) {
  test_for_missing_cache();
  test_for_missing_key();
  lock_cache();
