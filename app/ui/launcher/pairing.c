...
void _manual_add_window(struct nk_context *ctx) {
    ...
    struct nk_borders dec_size = nk_style_window_get_decoration_size(&ctx->style, NK_WINDOW_TITLE | NK_WINDOW_BORDER |
    ...
    if (nk_begin(ctx, "Add PC Manually", s, NK_WINDOW_TITLE | NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR)) {
        ...
        nk_flags editor_state = nk_edit_string(ctx, NK_EDIT_FIELD | NK_EDIT_GOTO_END_ON_ACTIVATE, text, &text_len, 64,
                                               nk_filter_ip);
...
void _pairing_window(struct nk_context *ctx) {
    ...
    int dialog_height =
            dec_size.t + message_height + ctx->style.window.spacing.y + 50 * NK_UI_SCALE + 12 * NK_UI_SCALE +
            dec_size.b;
    struct nk_rect s = nk_rect_centered(ui_display_width, ui_display_height, dialog_width, dialog_height);
    if (nk_begin(ctx, "Pairing", s,
                 NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_NOT_INTERACTIVE | NK_WINDOW_NO_SCROLLBAR)) {
        nk_layout_row_dynamic(ctx, message_height, 1);
        nk_label_wrap(ctx, message);
