...
int streaming_worker(session_t *session) {
    ...
    SS4S_PlayerSetViewportSize(session->player, app->ui.width, global->ui.height);
...
//                break;
    ...
    free(session);
    session_active = NULL;
    app_bus_post(app, (bus_actionfunc) streaming_thread_wait, thread);
    return 0;
}
